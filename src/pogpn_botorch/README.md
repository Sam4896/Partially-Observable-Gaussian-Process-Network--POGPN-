...
This package provides an implementation of the Partially Observable Gaussian Process Network (POGPN), a flexible model for structured multi-output regression, integrated with the BoTorch library. It is particularly well-suited for Bayesian Optimization tasks where there are intermediate, observable outputs that are causally related.
