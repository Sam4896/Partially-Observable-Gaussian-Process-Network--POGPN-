...
class DAG(nx.DiGraph):
    ...
    def plot_dag(self, save_dir=None, show: bool = False):
        """Plot the DAG in left-to-right orientation using Graphviz's dot layout."""
        try:
            node_positions = graphviz_layout(self, prog="dot", args="-Grankdir=LR")
        except (ImportError, Exception) as error:
            print(f"Warning: Could not use graphviz layout: {str(error)}")
            node_positions = nx.spring_layout(self, k=1, iterations=50)
